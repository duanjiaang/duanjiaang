...
for (i = 1990; i <= year; i++) {
  ...
  // 内容是年份
  op.append(i + '年'); //<option>xxxx年<option>
  // 修改属性
...
for (i = 1; i <= 12; i++) {
  ...
  op.setAttribute('value', i + '月');
  op.append(i + '月');
  selmonth.append(op);
}
for (i = 1; i < 32; i++) {
  var op = document.createElement('option');
  op.setAttribute('value', i + '日');
  op.append(i + '日');
  selday.append(op);
}
...
btnYear.addEventListener('click', function () {
  alert(selyear.value + selmonth.value + selday.value);
});
...
// 在页面上动态创建不少于五种类型的元素  e.g.  var ele = document.createElement('div');
